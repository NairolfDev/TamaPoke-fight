...
TEST_KNOWN_ISSUE(level, no_desborda_con_mascotas_longevas,
                 "level() devuelve uint8_t: a los ~10,6 dias de juego (ageMinutes >= 15300) "
                 "el nivel da la vuelta a 0 y la mascota 'rejuvenece'") {
  Pet p;
  makePet(p, 4);
  p.ageMinutes = 254 * 60;
...
TEST_KNOWN_ISSUE(evolve, muchos_descuidos_no_adelantan_la_evolucion,
                 "canEvolveNow() compara con (uint8_t)(evolveLevel + careMistakes): con muchos "
                 "descuidos la suma da la vuelta y la evolucion se desbloquea ANTES de tiempo "
                 "en vez de retrasarse") {
  Pet p;
  makePet(p, 4);       // CHARMANDER: evoluciona a nivel 16
...
TEST_KNOWN_ISSUE(train, el_saco_anuncia_lo_que_de_verdad_sube,
                 "trainStrength() devuelve la subida teorica, no la aplicada: al topar en 100 "
                 "la pantalla anuncia 'FUERZA +18' cuando solo han entrado los puntos que cabian") {
  Pet p;
  makePet(p, 4);
  p.trAtk = 95;
